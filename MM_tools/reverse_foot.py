...
def create_rfs(side, *args):
    ...
    # ankle to ball
    ankle_eff_name = cmds.ikHandle(n = side + '_ball_ikHandle', sj= side + '_ankle_IK_jnt', ee= side + '_ball_IK_jnt', sol='ikSCsolver', s = 'sticky')
    cmds.rename(ankle_eff_name[1], side + '_ball_effector')

    # ball to toe
    ball_eff_name = cmds.ikHandle(n= side + '_toe_ikHandle', sj= side + '_ball_IK_jnt', ee= side + '_toe_IK_jnt', sol='ikSCsolver', s = 'sticky')
    ...
    cmds.parent(side + '_pv_offset', side + '_foot_IK_ctl')
